...
int main(int argc, char *argv[]) {
  ...
  for(int t=0; t<nTime-1; t++) {
    evolvePDEMPI(pGrid, t, &processor);
  ...
  if (id==0) {
    ...
    writeDataMPI(dumpMemory,nTime,nPointsTotal,"dataFile.csv");
    printResults("resultsFile.csv",nPointsTotal,nProcs,dt,dx,duration.count());
    freeMemoryMPI(dumpMemory, nTime);
    if (showResults){
