...
void solver<dim>::makeGrid(const int &globalLevel) {
    ...
    if (dim==3) {
        GridTools::collect_periodic_faces(mesh, 4,5,1,periodicityVector);
    }
...
void solver<dim>::setupSystem(const bool &periodicBC) {
    ...
    if (periodicBC) {
        ...
        if (dim==3) {
            GridTools::collect_periodic_faces(dofHandler, 4,5,1,periodicityVector);
        }
...
int solver<dim>::solve() {
    ...
    SolverControl control(dofHandler.n_dofs(), 1e-7);
    LinearAlgebraPETSc::SolverCG solver(control, communicator);
    LinearAlgebraPETSc::MPI::PreconditionJacobi precond;
    LinearAlgebraPETSc::MPI::PreconditionJacobi::AdditionalData data;
    ...
    if (Utilities::MPI::this_mpi_process(communicator) == 0)
      {
        ...
        std::ofstream masterOutput("solution-" + Utilities::int_to_string(cycle, 2) + ".pvtu");
